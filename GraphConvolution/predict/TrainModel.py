...
def Save_Model(model, auc_val_final, model_name):
    print('\n')
    print(f'Validation accuracy: {auc_val_final:.4f}')
    # get the current path
    current_path = os.path.abspath(os.path.dirname(__file__))
    model_path = os.path.join(current_path, 'models')
    if auc_val_final > 0.85:
        torch.save(model.state_dict(), f'{model_path}/GCNNet_{model_name}_auc{auc_val_final:.5f}.pth')
        print(f'Model saved in {model_path}/GCNNet_{model_name}_auc{auc_val_final:.5f}.pth')


def validation_test(model, criterion, torch_dataset, original_dataset):
    with torch.no_grad():
        model.eval()
        val_positive_edge_pairs = torch_dataset.Edge_pairs[torch_dataset.val_mask].t()
        val_negative_edge_pairs = original_dataset.negative_sampling(val_positive_edge_pairs)
...
def generate_true_labels(posi_edge_pairs, nega_edge_pairs):
    ...
    labels = torch.zeros(num_pairs, dtype=torch.float)
...
def train_model(epochs, model, criterion, optimizer, scheduler, torch_dataset, original_dataset):
    ...
    for epoch in range(epochs):
        model.train()
        train_positive_edge_pairs = torch_dataset.Edge_pairs[torch_dataset.train_mask].t()
        train_negative_edge_pairs = original_dataset.negative_sampling(train_positive_edge_pairs)
        output = model(torch_dataset.features, torch_dataset.Matrix_sparse, \
    ...
    recorder.Plot_loss_auc(epochs, array_train_loss, array_val_loss, array_train_auc, \
                            array_val_auc, learning_rate, original_dataset.dataset)
    recorder.Output_logs(array_train_loss, array_val_loss, array_train_auc, array_val_auc, original_dataset.dataset)
    Save_Model(model, array_val_auc[-1], original_dataset.dataset)
